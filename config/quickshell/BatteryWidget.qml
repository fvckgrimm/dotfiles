...
Item {
    ...
    readonly property string batColor: {
        if (isCharging || isFull) return Theme.success
    ...
    Process {
        ...
        running: true
        stdout: SplitParser { onRead: data => { var v = parseInt(data.trim()); if (!isNaN(v)) root.capacity = v } }
    }
    Process {
        id: statusProc
    ...
    Timer { interval: 2000; running: true; repeat: true; onTriggered: { capProc.running = true; statusProc.running = true } }
