...
Item {
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: data => {
                var s = data.trim()
                if (s === "DISCONNECTED" || s === "") { root.connected = false; root.display = " "; root.tooltip = "No network"; return }
                root.connected = true
                var parts = s.split("|")
                if (parts[0] === "WIFI") {
    ...
    StatChip {
        ...
        valueColor: root.connected ? Theme.primary : Theme.error
        ...
        onClicked: Quickshell.exec(["bash", "-c", "ip address show up scope global | grep inet | head -n1 | cut -d/ -f1 | awk '{print $2}' | wl-copy"])
        onRightClicked: Quickshell.exec(["bash", "-c", "ip address show up scope global | grep inet6 | head -n1 | cut -d/ -f1 | awk '{print $2}' | wl-copy"])
    }
}
