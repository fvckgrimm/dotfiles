...
Item {
    ...
    Process {
        ...
        running: true
        stdout: SplitParser { onRead: data => { var v = parseInt(data.trim()); if (!isNaN(v)) root.tempC = v } }
    }
    ...
    StatChip {
        ...
        icon: root.tempIcon
        value: root.tempC + "°"
        iconColor: root.isCritical ? Theme.error : Theme.surfaceTextVariant
        valueColor: root.isCritical ? Theme.error : Theme.surfaceTextVariant
